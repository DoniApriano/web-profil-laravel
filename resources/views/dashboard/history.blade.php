...
@section('content')
            ...
            <div class="card-body">
                <div class="mb-3">
                    <div class="text-center">
                        <img class="img-fluid" id="history-image-preview"
                            src="/storage/history-image/{{ $history->image }}" alt="{{ $history->title }}">
                    </div>
                    <label for="image" class="form-label">Gambar</label>
                    <input id="image" type="file" value="{{ $history->image }}" name="image"
                        class="form-control" autofocus>
                    <div class="alert alert-danger mt-2 d-none" role="alert" id="alert-image"></div>
                </div>
                <div class="mb-3">
                    <label for="title" class="form-label">Judul</label>
                    <input id="title" type="text" value="{{ $history->title }}" name="title"
                        class="form-control" autofocus>
                    <div class="alert alert-danger mt-2 d-none" role="alert" id="alert-title"></div>
                </div>
                <div class="mb-3">
                    <label for="text" class="form-label">Sambutan</label>
                    <textarea id="text" type="text" name="text" class="form-control" autofocus>{{ $history->text }}</textarea>
                    <script>
    ...
    <script>
        $('#btn-store').click(function(e) {
            ...
            var createRoute = "{{ route('history.store') }}";
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#history-image-preview').attr('src', '/storage/history-image/' + response.data.image);
                    Swal.fire({
                        type: 'success',
                        icon: 'success',
